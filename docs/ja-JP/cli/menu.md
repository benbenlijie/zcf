...
`npx @benbenwu/zcf` を実行すると、対話式メニューシステムに入ります。メニューは ZCF のコアインターフェースで、視覚的な操作オプションを提供し、複雑なコマンドパラメータを覚える必要がありません。
...
`npx @benbenwu/zcf init` を実行するのと同じで、以下を案内します：
...
`npx @benbenwu/zcf update` を実行するのと同じで、以下を実行します：
...
# 2. 手動で切替
npx @benbenwu/zcf init -T codex  # Codex に切替
npx @benbenwu/zcf init -T claude-code  # Claude Code に切替
```
