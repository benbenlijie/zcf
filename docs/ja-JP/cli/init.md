...
# 設定処理方法
npx @benbenwu/zcf i -s --config-action backup  # バックアップ後に上書き（デフォルト）
npx @benbenwu/zcf i -s --config-action merge   # 設定をマージ
npx @benbenwu/zcf i -s --config-action new     # 新しい設定を作成
npx @benbenwu/zcf i -s --config-action docs-only  # ドキュメントのみ更新
npx @benbenwu/zcf i -s --config-action skip    # 設定をスキップ

# CCometixLine インストールを制御
npx @benbenwu/zcf i -s -x true   # インストール（デフォルト）
npx @benbenwu/zcf i -s -x false  # インストールしない
```
