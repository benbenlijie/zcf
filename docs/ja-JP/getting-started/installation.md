...
> 💡 **ヒント**: ZCFはインストール不要で、`npx @benbenwu/zcf`を直接実行できます。このガイドは主に環境設定と使用フローについて説明しています。
...
> 💡 **ヒント**: ZCFはインストール不要で、`npx @benbenwu/zcf`コマンドを直接実行できます。
...
# 方法2: 対話式メニュー経由
npx @benbenwu/zcf → S（ツール切り替え）を選択 → 1（完全初期化）を選択
```

### Codex設定の特徴

- **設定ファイル**: `~/.codex/config.toml`（TOML形式）
- **ワークフロー場所**: `~/.codex/skills/zcf-*`
...
> ⚠️ **注意**: WindowsでMCP接続の問題が発生した場合、`npx @benbenwu/zcf`を実行すると設定形式が自動的に修正されます。
...
**Codex:**
```
$zcf-six-step <タスク説明>  # 6段階開発ワークフロー
$zcf-git-commit             # Gitコミットコマンド
$zcf-git-rollback           # Gitロールバックコマンド
$zcf-git-clean-branches     # マージ済みブランチのクリーンアップ
$zcf-git-worktree           # Git worktree管理
```
...
> 
> 💡 **ヒント**: Codexは`skills`を使用します。先に`/skills`を入力すると導入済み skill を確認できます。Claude Codeは`/zcf:`または直接`/`プレフィックスを使用します。
...
> 🔧 **トラブルシューティング**: サービスが接続されていない場合、`npx @benbenwu/zcf` → `4`を実行してMCPサービスを再設定してください。
...
ファイルが存在しない場合、`npx @benbenwu/zcf update`を実行して再インポートしてください。

### Q: MCPサービスの接続に失敗しましたか？

**A**: 
1. MCPサービス設定を確認: `npx @benbenwu/zcf` → `4`
...
**A**: 設定切り替えコマンドを使用：
```bash
npx @benbenwu/zcf config-switch --list  # すべての設定を一覧表示
npx @benbenwu/zcf cs provider-name      # 指定された設定に切り替え
```
