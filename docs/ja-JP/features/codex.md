...
# カスタムエンドポイント
npx @benbenwu/zcf init -s -T codex -t api_key -k "sk-xxx" -u "https://api.example.com/v1" -M "gpt-4.1"
```
...
```bash
# Codex 関連を選択削除
npx @benbenwu/zcf uninstall --mode custom --items codex
```
...
- 問題が起きたらバックアップから復元し、`npx @benbenwu/zcf init -T codex --config-action merge` で再適用してください。
