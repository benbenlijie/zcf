...
6段階ワークフローは ZCF のコア開発プロセスであり、完全なソフトウェア開発ライフサイクルをカバーします：**リサーチ → 構想 → 計画 → 実行 → 最適化 → レビュー**。
...
**例**：
```
/zcf:workflow ユーザーログイン機能を実装、メールと電話番号ログインをサポート
```

### Codex

Codex では以下の skill 呼び出しを使用します：
...
**例**：
```
$zcf-six-step ユーザーログイン機能を実装、メールと電話番号ログインをサポート
```

> 💡 **ヒント**：Codex は `skills` を使います。先に `/skills` を入力すると導入済み skill を確認できます。Claude Code は `/zcf:` プレフィックスを使用します。
...
- 技術実装計画
- 開発計画とタイムライン
- 計画ドキュメント（`.zcf/plan/current/タスク名.md` に保存）
...
1. **コマンド入力**：`/zcf:workflow` または `$zcf-six-step` を入力してタスクを説明
...
- **進行中のタスク**：`.zcf/plan/current/` ディレクトリ
- **完了したタスク**：`.zcf/plan/history/` ディレクトリ

> 💡 **ヒント**：`.zcf/` は統一ワークフローディレクトリであり、Claude Code でも Codex でも同じパスを使用します。
...
```
# メインタスク
/zcf:workflow ユーザー管理システムを構築

# サブタスク 1
/zcf:workflow ユーザー登録機能を実装

# サブタスク 2
/zcf:workflow ユーザーログイン機能を実装

# サブタスク 3
/zcf:workflow ユーザー情報管理を実装
```
...
# 2. 6段階ワークフローで詳細を実装
/zcf:workflow コメントの CRUD 操作と権限制御を実装
...
```
/zcf:workflow ユーザーアバターアップロード機能を追加
```
...
```
/zcf:workflow マイクロサービスアーキテクチャのユーザー認証システムを構築、OAuth2 と JWT をサポート
```
...
```
/zcf:workflow ユーザーモジュールをリファクタリング、コード品質と保守性を向上
```
...
ワークフローは統一された `.zcf/plan/` ディレクトリ構造を使用：
...
- ⚠️ **統一ディレクトリ**：Claude Code でも Codex でも `.zcf/plan/` ディレクトリを使用
...
> 💡 **ヒント**：6段階ワークフローは ZCF のコア機能であり、ほとんどの開発タスクに適しています。小さなタスクから始めてプロセスに慣れ、その後徐々により複雑なプロジェクトに適用することを推奨します。段階確認メカニズムを適切に使用することで、より良い開発体験が得られます。
