...
|------|------------|-------|------|
| **智能提交** | `/git-commit` | `$zcf-git-commit` | 智能生成提交信息并拆分提交 |
| **安全回滚** | `/git-rollback` | `$zcf-git-rollback` | 安全回滚到指定提交 |
| **清理分支** | `/git-cleanBranches` | `$zcf-git-clean-branches` | 清理已合并分支 |
| **工作树管理** | `/git-worktree` | `$zcf-git-worktree` | 管理 Git Worktree，支持创建、迁移、删除 |

> 💡 **提示**：Codex 通过 `skills` 调用这些 Git 工作流，可先输入 `/skills` 查看已安装技能；Claude Code 直接使用 `/` 前缀。
...
# 或 Codex 中：
$zcf-git-worktree 新建 feat/add-i18n 并打开
```
