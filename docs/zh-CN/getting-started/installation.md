...
> 💡 **提示**：ZCF 无需安装，直接使用 `npx @benbenwu/zcf` 即可运行。本指南主要介绍环境配置和使用流程。
...
> 💡 **提示**：ZCF 无需安装，直接使用 `npx @benbenwu/zcf` 命令即可运行。
...
# 方式 2：通过交互式菜单
npx @benbenwu/zcf → 选择 S（切换工具）→ 选择 1（完整初始化）
```
...
> ⚠️ **注意**：如果在 Windows 上遇到 MCP 连接问题，运行 `npx @benbenwu/zcf` 会自动修复配置格式。
...
> 🔧 **故障排除**：如果服务未连接，运行 `npx @benbenwu/zcf` → `4` 重新配置 MCP 服务。
...
如果文件不存在，运行 `npx @benbenwu/zcf update` 重新导入。

### Q: MCP 服务连接失败？

**A**: 
1. 检查 MCP 服务配置：`npx @benbenwu/zcf` → `4`
...
**A**: 使用配置切换命令：
```bash
npx @benbenwu/zcf config-switch --list  # 列出所有配置
npx @benbenwu/zcf cs provider-name      # 切换到指定配置
```
