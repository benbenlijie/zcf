...
- 工作流会自动安装到 Claude Code 的 `~/.claude/workflows/`，以及 Codex 的 `~/.codex/skills/zcf-*`。
...
> 💡 **提示**：Codex 通过 `skills` 调用工作流。可先输入 `/skills` 查看已安装技能，而 Claude Code 使用 `/zcf:` 前缀或直接 `/` 前缀。

## 使用建议

- 首次使用工作流时，可让 AI 输出任务进度文档，方便后续在新对话继续。Claude Code 使用 `/zcf:workflow <任务描述>`，Codex 使用 `$zcf-six-step <任务描述>`。
